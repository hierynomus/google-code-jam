...
object AlienLanguage {
  ...
  def main(args: Array[String]) {
    ...
    val firstLine = iterator.nextIntArray
    val (l, nrWords, nrProblems) = (firstLine(0), firstLine(1), firstLine(2))
    val words = for (i <- 1 to nrWords) yield iterator.next()

    output.writeStrings((1 to nrProblems).map(p => "Case #" + p + ": " + solveProblem(words, iterator)), "\n")
  }
