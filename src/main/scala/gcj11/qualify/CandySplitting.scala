...
object CandySplitting {
  import GoogleCodeHelper.iteratorToHelper

  def solveProblem(reader: Iterator[String]) = {
    reader.skipLines(1)
    val sortedCandy = reader.nextIntArray.toList.sorted(Ordering.Int)
    if (sortedCandy.reduceLeft(_^_) == 0) sortedCandy.tail.sum.toString else "NO"
  }

  def name = "gcj11.qualify.CandySplitting"
